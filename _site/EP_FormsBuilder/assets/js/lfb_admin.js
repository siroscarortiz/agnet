...
jQuery(document).ready(function () {
    ...
    jQuery('.imageBtn').click(function () {
        lfb_openUploadPic(jQuery(this).prev('input'));
        return false;
    });
    jQuery('#wpwrap').css({
    ...
    if (jQuery('#lfb_winActivation').is('[data-show="true"]') && document.referrer.indexOf('EP_FormsBuilder') < 0) {
        lfb_lock = true;
...
function lfb_addStep(step) {
   // lfb_showLoader();
    ...
    if (jQuery('#lfb_stepsContainer .lfb_stepBloc').length == 0) {
        ...
        newStep.uniqueId();
        var posX = jQuery('#lfb_stepsOverflow').scrollLeft() + jQuery('#lfb_stepsOverflow').width() / 2 - 64;
        var posY = jQuery('#lfb_stepsOverflow').scrollTop() + jQuery('#lfb_stepsOverflow').height() / 2 - 64;
        jQuery.ajax({
            url: ajaxurl,
            type: 'post',
            data: {
...
function lfb_saveItem() {
    ...
    jQuery('#lfb_winItem').find('input[type="number"]').each(function(){
       if(jQuery(this).val() == ''){
           jQuery(this).val(0);
       } 
    });
...
function lfb_loadForm(formID) {
  ...
  //  lfb_showLoader();
    ...
    jQuery('#lfb_btnPreview').attr('href','viewForm.php?lfb_action=preview&form=' + formID);
    jQuery.ajax({
        url: ajaxurl,
        type: 'post',
        data: {
        ...
        success: function (rep) {
           ...
           // lfb_redirConditionsTable
            ...
            setTimeout(function(){
               jQuery('.note-image-btn').removeAttr('disabled'); 
               jQuery('.note-image-btn').removeClass('disabled'); 
            },300);
            
            if(lfb_data.designForm != 0){
                
                lfb_data.designForm = 0;
                window.history.pushState('lfb', document.title, document.location.href.replace('?lfb_formDesign='+lfb_currentFormID,''));
    ...
    if (!error) {
        ...
        jQuery.ajax({
            ...
            success: function (rep) {
                ...
                jQuery('#lfb_winEditCoupon').fadeOut();
...
function lfb_addonTdgn(){
    ...
    jQuery.ajax({
        ...
        success:function(rep){
            rep.trim();
            if(rep == '101') {
                document.location.href = document.location.href.substr(0,document.location.href.lastIndexOf('/')+1) +'?lfb_formDesign='+lfb_currentFormID;
            }else {
                jQuery('#lfb_loader').fadeOut();
...
function lfb_openUploadPic($field){
    lfb_uploadField = $field;
    jQuery('#lfb_winUploadPic').modal('show');
}
function lfb_importPicture(){
    lfb_showLoader();
    jQuery('#lfb_winUploadPic').modal('hide');
    var formData = new FormData(jQuery('#lfb_winUploadPicForm')[0]);
    ...
    jQuery.ajax({
        ...
        success: function (rep) {
            ...
            rep = rep.trim();
            if(rep.indexOf('.')==-1){
                alert('The plugin can\'t import the file, please set the uploads/ folder permissions to 0745 using a FTP software');
            } else {
                lfb_uploadField.val('uploads/'+rep);
            }
        },
        data: formData,
        cache: false,
