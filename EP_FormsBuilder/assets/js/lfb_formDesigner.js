...
function tld_onOpen(){
    tld_previewUrl = 'viewForm.php?lfb_action=preview&form='+lfb_currentFormID+'&rand='+Math.random();
...
function tld_tdgn_init() {
    ...
    jQuery('#tld_tdgnFrame').on('load', function () {
        if(typeof jQuery('#tld_tdgnFrame').get(0).contentWindow.tld_unSelectElement == 'function'){
        if(tld_firstLoad){
            tld_firstLoad = false;
        }    
